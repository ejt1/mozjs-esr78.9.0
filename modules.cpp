...
static JSObject* ExampleResolveHook(JSContext* cx,
  ...
  if (!spec) {
    JS_ReportErrorUTF8(cx, "cannot flatten null string");
    return false;
  }
  
  JSLinearString* linear = JS_EnsureLinearString(cx, spec);
  if (!linear) {
    JS_ReportErrorUTF8(cx, "failed to flatten string");
    return false;
  }
  size_t len = JS::GetDeflatedUTF8StringLength(linear);
  char* str = new char[len + 1];
  JS::DeflateStringToUTF8Buffer(linear, mozilla::Span<char>(str, len));
  str[len] = '\0';
